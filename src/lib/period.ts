...
// BRT is UTC-3; the same cutoff hour expressed in UTC:
const UTC_CUTOFF_HOUR = BRT_CUTOFF_HOUR + 3
...
export function opDateRangeUTC(opDate: string): { from: string; to: string } {
  const cutoffStr = `T${pad2(UTC_CUTOFF_HOUR)}:00:00.000Z`
  const next = new Date(opDate + 'T00:00:00Z')
  next.setUTCDate(next.getUTCDate() + 1)
  return { from: opDate + cutoffStr, to: formatUTCDate(next) + cutoffStr }
}
...
export function periodRangeUTC(start: string, end: string): { from: string; to: string } {
  ...
  return {
    from: start + cutoffStr,
    to: formatUTCDate(endNext) + cutoffStr,
  }
}
