...
export function computeDashboard(
  ...
  const caixaInicial = extractCaixaTotal(firstSession?.initial_counts ?? null)
  ...
  const saidas = entries
    .filter((e) => e.type === 'expense' || e.type === 'withdrawal')
    .reduce((s, e) => s + Number(e.amount), 0)

  const caixaFinal = extractCaixaTotal(lastSession?.final_counts ?? null)
...
export function computeHoldingDashboard(
  ...
  const caixaInicial = sessions.reduce(
    (s, sess) => s + extractCaixaTotal(sess.initial_counts),
    0
  )
  const caixaFinal = sessions.reduce(
    (s, sess) => s + extractCaixaTotal(sess.final_counts),
    0
  )
