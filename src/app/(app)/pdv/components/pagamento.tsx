...
type PayBtnProps = {
  p: ConfigItem;
  forma: string;
  setForma: (f: string) => void;
  trocoRef: React.RefObject<HTMLInputElement | null>;
};

function PayBtn({ p, forma, setForma, trocoRef }: PayBtnProps) {
    ...
    <div style={{ width: WRAP_W }}>
      ...
      >
        ...
        {ativas.length === 0 ? (
          ...
          >
            {ativas.map((p) => (
              <PayBtn key={p.id} p={p} forma={forma} setForma={setForma} trocoRef={trocoRef} />
            ))}
