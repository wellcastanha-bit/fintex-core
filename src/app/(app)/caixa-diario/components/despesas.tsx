      ...
      {React.Children.map(props.children, (child) => {
        ...
        if (child.type === "option") {
          return React.cloneElement(child as any, {
            style: { backgroundColor: "#0b192d", color: "#eaf0ff" },
          });
        }
        return child;
      })}
...
type Props = {
  ...
  setExpensePaymentMethod: React.Dispatch<React.SetStateAction<string>>;
  addExpense: () => void;
...
export default function DespesasTab({
  ...
  const [hoverField, setHoverField] = useState<"cat" | "valor" | "desc" | "pay" | null>(null);

  const regCardGlowOn = hoverRegCard && !hoverField;
  const catRef = useRef<HTMLSelectElement | null>(null);
  const payRef = useRef<HTMLSelectElement | null>(null);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 text-slate-300 transition-colors duration-150 group-hover:text-white">
                            {e.payment_method ? (PAYMENT_METHOD_LABEL[e.payment_method] ?? e.payment_method) : <span className="text-slate-500/60">—</span>}
                          </td>
