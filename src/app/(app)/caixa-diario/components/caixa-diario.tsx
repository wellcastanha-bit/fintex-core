...
export default function CaixaDiario() {
      ...
      try {
        ...
        for (const e of entries) {
          ...
          if (e.type === "manual_in") {
            ...
            exps.push({ id: e.id, date: dateISO, time, category: String(e.category || ""), description: String(e.description || ""), amount: amt });
  ...
  const esperado = useMemo(
    () => caixaInicial + cashInTotal - totalDespesas - totalSangrias,
    [caixaInicial, cashInTotal, totalDespesas, totalSangrias]
  );
  ...
  const addExpense = async () => {
    ...
    try {
      const res = await fetch("/api/caixa-diario/entries", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ op_date: dateISO, type: "expense", amount: v, category: optimistic.category, description: optimistic.description }),
      });
      if (!res.ok) throw new Error();
      const data = await res.json();
      const e = data.entry;
