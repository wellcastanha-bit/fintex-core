...
export default function PedidosClient({
    ...
    for (const nr of nextRows) {
      ...
      (async () => {
        try {
          ...
          // Rollback: restore this row to its state before the optimistic update
          if (prevRow) {
            setRowsState((current) =>
              current.map((r) => (String(r?.__ID ?? "") === id ? prevRow : r))
            );
            lastSentRef.current.delete(id);
          }
          if (patchErrorTimerRef.current) clearTimeout(patchErrorTimerRef.current);
          setPatchError("Erro ao salvar alteração. Dado revertido.");
  ...
  const autosomasGerais = useMemo(() => {
    ...
    for (const r of base) {
      qtdePedidos += 1;
      const v = parseBRLToNumber(r?.[normKey("R$ FINAL")]);
  ...
  return (
    <div style={{ width: "100%", minWidth: 0, boxSizing: "border-box", position: "relative" }}>
      {patchError && (
        <div style={{ padding: "10px 14px", background: "rgba(231,76,60,0.15)", border: "1px solid rgba(231,76,60,0.5)", borderRadius: 8, marginBottom: 8, color: "#e74c3c", fontWeight: 700, fontSize: 13 }}>
          {patchError}
        </div>
      )}
