...
import { RESPONSAVEIS, STATUS_OPTS, type Row, type ColDef } from "./pedidos.constant";
...
export default function PedidosTable(props: Props) {
  const { filtered, cols, minWidth, highlightSetRef, setHighlightIds, confirmBusy, onRequestDelete, setRows } = props;

  const [hoverTable, setHoverTable] = useState(false);
  const [hoverHeader, setHoverHeader] = useState(false);
  const [trashHoverId, setTrashHoverId] = useState<string>("");

  const totalW = useMemo(() => cols.reduce((a: number, c: ColDef) => a + c.w, 0), [cols]);
        ...
        >
          ...
          >
            {cols.map((c: ColDef) => {
              ...
              const isEditable = k === normKey("RESPONSÁVEL") || k === normKey("STATUS") || c.type === "number";
